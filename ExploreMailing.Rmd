...
###This report explores the Gainesville OwnedBy mailing list and various diagnostic graphs.
...
fln = paste0("Latino-OwnInv.MAILING.07022015",".xls") # MAILING FILE TO INVESTIGATE
OWNERSHIP = c("Owner","Invester","Renter")
Ownedby = OWNERSHIP[1]
#
...
OwnedBys=readWorksheetFromFile(file = fln, sheet = Ownedby)
print(getwd())
cat("Read ",Ownedby, " mailing list to OwnedBys dataframe of: ",nrow(OwnedBys)," Records \n")
```
...
Use = unique(OwnedBys$PUse)
Start = 1 # 1= single family
End = length(Use)
#End = 3
...
for(jj in seqi){
  ResidentType = OwnedBys[OwnedBys$PUse == Use[jj],]
  ...
  cat(Use[jj]," Total Top 20 Neighborhoods Count: ",sum(leadingNeighborhoods))
  nn = names(leadingNeighborhoods)
  bw = 1
  ml = paste0(Use[jj]," Neighborhoods")
  ResidentTypeAll = ResidentType
  ...
  cat("% of ", Use[jj]," ",per,"% \n")
  x = sort(table(ResidentType$Neighborhood),decreasing=TRUE)  
  print(round(x/nrow(ResidentType)*100,1))
  x = sort(table(ResidentTypeAll$Neighborhood),decreasing=TRUE)  
  print(x)
